...
class JobVacancyC extends Controller
{
  ...
  public function index($id = null)
  {
    ...
    $page_title = "JobVacancy";
    $page_route = $this->page_route;
    $data = compact('rows', 'sd', 'ft', 'url', 'title', 'page_title', 'page_route', 'page_no');
    return view('admin.job-vacancy')->with($data);
  }
  public function getData(Request $request)
  {
    $rows = JobVacancy::where('id', '!=', '0');
    ...
    if ($rows->count() > 0) {
      foreach ($rows as $row) {
        ...
        $status = $row->status == 1 ? 'Avtive' : 'Inactive';
