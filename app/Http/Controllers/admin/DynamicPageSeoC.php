...
class DynamicPageSeoC extends Controller
{
  public function index($id = null)
  {
    ...
    if ($id != null) {
      ...
      if (!is_null($sd)) {
        $ft = 'edit';
        $url = url('admin/dynamic-page-seo/update/' . $id);
        $title = 'Update';
      } else {
        return redirect('admin/dynamic-page-seo');
      }
    } else {
      $ft = 'add';
      $url = url('admin/dynamic-page-seo/store');
      $title = 'Add New';
      $sd = '';
    }
    $page_title = "Dynamic Page Seo";
    $page_route = "dynamic-page-seo";
    $data = compact('rows', 'sd', 'ft', 'url', 'title', 'page_title', 'page_route', 'page_no');
    return view('admin.dynamic-page-seo')->with($data);
  }
  public function getData(Request $request)
  {
    $rows = DynamicPageSeo::where('id', '!=', '0');
    $rows = $rows->paginate(10)->withPath('/admin/dynamic-page-seo/');
    $i = 1;
