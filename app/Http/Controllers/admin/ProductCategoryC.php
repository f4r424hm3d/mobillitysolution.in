...
class ProductCategoryC extends Controller
{
  public function index($id = null)
  {
    $page_no = $_GET['page'] ?? 1;
    $rows = ProductCategory::get();
    if ($id != null) {
      $sd = ProductCategory::find($id);
      if (!is_null($sd)) {
        $ft = 'edit';
        $url = url('admin/product-categories/update/' . $id);
        $title = 'Update';
      } else {
      ...
      $ft = 'add';
      $url = url('admin/product-categories/store');
      $title = 'Add New';
      $sd = '';
    }
    $page_title = "Product Categories";
    $page_route = "product-categories";
    $data = compact('rows', 'sd', 'ft', 'url', 'title', 'page_title', 'page_route', 'page_no');
    return view('admin.product-categories')->with($data);
  }
  public function getData(Request $request)
  {
    $rows = ProductCategory::where('id', '!=', '0');
    $rows = $rows->paginate(10)->withPath('/admin/product-categories/');
    $i = 1;
    ...
    if ($rows->count() > 0) {
      ...
      <td>
        ...
        <a href="' . url("admin/product-categories/update/" . $row->id) . '"
