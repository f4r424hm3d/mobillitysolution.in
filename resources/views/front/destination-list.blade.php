        ...
        <div class="col-lg-12 col-md-12">
          <div class="ed_detail_wrap light">
            <ul class="cources_facts_list">
              ...
              <li class="facts-1">All Universities</li>
            </ul>
                                ...
                                <ul>
                                  @php
                                    $states = University::select('state', 'state_slug')
                                        ->where(['status' => 1, 'destination_id' => $row->destination_id])
                                        ->where('state', '!=', null)
                                        ->groupBy('state')
                                        ->limit(20)
                                        ->get();
                                  ...
                                  @foreach ($cities as $city)
                                    <li><a
                                        href="{{ url($row->getDestination->destination_slug . '/' . $city->city_slug . '-universities') }}">{{ $city->city }}</a>
                                    </li>
                                  @endforeach
                                </ul>
                              </div>
